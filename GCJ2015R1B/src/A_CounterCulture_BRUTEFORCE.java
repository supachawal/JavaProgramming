...
public class A_CounterCulture_BRUTEFORCE {
...
//			inputFileName = "-large-practice.in";
			inputFileName = A_CounterCulture_BRUTEFORCE.class.getSimpleName().substring(0, 1) + inputFileName;
		}
		...
		try {
			...
			A_CounterCulture_BRUTEFORCE solver = new A_CounterCulture_BRUTEFORCE();
	...
	HashMap<LongIntPair, Long> memoization = new HashMap<LongIntPair, Long>(8192);
	long N;
...
////////////////////////////////////////////////////////////////////////////////////
	...
	private long minNumCount(long from) {
		if (from == N)
			return 1;
		...
		LongIntPair memoKey = new LongIntPair(from, N);

		if (memoization.containsKey(memoKey)) {
			return memoization.get(memoKey);
		}
		iterationCounter++;
		long min = Math.min(minNumCount(from + 1)
							, minNumCount(reverseNum(from))
							);
		
		memoization.put(memoKey, 1 + min);
		return 1 + min;
	}
}