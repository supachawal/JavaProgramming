/* Problem C. gCampus (all-pairs shortest path)  The large problem's correct seems to be wrong! 555
...
//			inputFileName = "-large-practice.in";
			inputFileName = C_gCampus.class.getSimpleName().substring(0, 1) + inputFileName;
		}
		...
		try {
			...
			C_gCampus solver = new C_gCampus();
...
////////////////////////////////////////////////////////////////////////////////////
	...
	public StringBuilder inefficientRoadList_FloydWarshall(long[][] adjMatrix, int n, ArrayList<IntPack> roadList) {
		StringBuilder result = new StringBuilder();
		int i, j, k;
		long[][] a = new long[n][n];
		long[][] opt = new long[n][n];
		
		CommonUtils.copy2dArray(adjMatrix, a);
		long newCost;
		...
		// try to remove road if no gain
		long[][] b = new long[n][n];
		CommonUtils.copy2dArray(adjMatrix, b);
		CommonUtils.copy2dArray(a, opt);
		
		int roadCount = roadList.size();
		for (int r = 0; r < roadCount; r++) {
			int [] v = roadList.get(r).values;
			CommonUtils.copy2dArray(b, a);

			a[v[0]][v[1]] = CommonUtils.LONG_INFINITY;
			a[v[1]][v[0]] = CommonUtils.LONG_INFINITY;
			...
			// Sum the cost and also check disconnection
			boolean canCut = true;
			for (i = 0; i < n && canCut; i++) {
				for (j = i; j < n; j++) {
					if (a[i][j] > opt[i][j]) {
						canCut = false;
						break;
			...
			if (canCut) {
				result.append('\n').append(r);
				b[v[0]][v[1]] = CommonUtils.LONG_INFINITY;
				b[v[1]][v[0]] = CommonUtils.LONG_INFINITY;
			}
		}
		return result;
	}
