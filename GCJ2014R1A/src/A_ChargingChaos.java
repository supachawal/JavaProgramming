...
//			inputFileName = "-small-practice.in";
			...
			inputFileName = A_ChargingChaos.class.getSimpleName().substring(0, 1) + inputFileName;
		}
		...
		try {
			...
			A_ChargingChaos solver = new A_ChargingChaos();
...
//		iterateBits(0, 3, 1, 0);
		...
		try {
			...
			while (++testCaseNumber <= testCaseCount && (textLine = br.readLine()) != null) {
				...
				bitCount = Integer.valueOf(splitted[1]);

				textLine = br.readLine();
				splitted = textLine.split("\\s+");
				outlets = new long[n];
				for (i = 0; i < n; i++) {
					outlets[i] = Long.valueOf(splitted[i], 2);
				}
				...
				devices = new long[n];
				for (i = 0; i < n; i++) {
					devices[i] = Long.valueOf(splitted[i], 2);
				}
...
//				}
				...
				iterationCounter = 0;
				initialAnswer = minSwitchesToBeFlipped(n, bitCount, outlets, devices);
...
////////////////////////////////////////////////////////////////////////////////////
	...
	private int minSwitchesToBeFlipped(int n, int bitCount, long[] outlets, long[] devices) {
		int i, j;
		HashSet<Long> deviceHashSet = new HashSet<Long>(150);
		int result = Integer.MAX_VALUE;
		
		for (i = 0; i < n; i++) {
			deviceHashSet.add(devices[i]);
		}
		
		for (i = 0; i < n; i++) {
			long flippingNum = devices[i] ^ outlets[0];
			boolean containsAll = true;
			
			for (j = 1; j < n; j++) {
				iterationCounter++;

				if (!deviceHashSet.contains(outlets[j] ^ flippingNum)) {
					containsAll = false;
					break;
				}
			}
			...
			if (containsAll) {
				result = Math.min(result, Long.bitCount(flippingNum));
				if (result == 0) {
					break;
				}
			}
