...
@memoize
def minimum_hamiltonian_circuit_cost(sortedStoreSetTuple, gasPrice, perishabiltyFlagTuple):
# not yet finished
    if gasPrice == 0:
        return 0.0
    
    distance = 0
    n = len(sortedStoreSetTuple)
    pos = origin_pos
    for i in range(n):
        store = sortedStoreSetTuple[i]
        distance += inline_compute_distance(pos, store.position_xy)
        if perishabiltyFlagTuple[i]:
            distance += inline_compute_distance(pos, store.position_xy)
            pos = origin_pos
        else:
            pos = store.position_xy

    return distance * gasPrice

@memoize
def append_store_set_tuple(sortedStoreSetTuple, perishabiltyFlagTuple, itemStoreInfo, perishable):
    s = list(sortedStoreSetTuple)
    
# @memoize
# def minimum_shopping_cost(items, itemIndex, sortedStoreSetTuple, gasPrice, perishabiltyFlagTuple):
#     if itemIndex < 0:
#         return minimum_hamiltonian_circuit_cost(sortedStoreSetTuple, gasPrice, perishabiltyFlagTuple)
# 
#     minCost = INFINITY
#     for itemStoreInfo in items[itemIndex].get_store_info_list():
#         cost = itemStoreInfo.price
#         newSortedStoreSetTuple, newPerishabiltyFlagTuple = append_store_set_tuple(sortedStoreSetTuple
#                                                                                   , perishabiltyFlagTuple
#                                                                                   , itemStoreInfo
#                                                                                   , items[itemIndex].is_perishable
#                                                                                   )
#         cost += minimum_shopping_cost(items, itemIndex - 1, newSortedStoreSetTuple, gasPrice, newPerishabiltyFlagTuple)  
#         
#         if cost < minCost:
#             minCost = cost
# 
...
def minimum_shopping_cost(items, itemIndex, gasPrice):
...
#         print(pattern)
    ...
    ret = 0
    for itemStoreInfo in items[itemIndex].get_store_info_list():
#         pattern.append((items[itemIndex].name, itemStoreInfo.store_id, itemStoreInfo.price))
        ret = minimum_shopping_cost(items, itemIndex - 1, gasPrice)  
#         pattern.pop()
...
#         answer = minimum_shopping_cost(tuple(items), num_items - 1, tuple(), price_of_gas, tuple())
#         print('answer=%.7f' % (answer), flush=True)
        print()
        pattern_count = 0
        answer = minimum_shopping_cost(items, num_items - 1, price_of_gas)
