...
def getBarber(B     # barber count
             , N    # customer index (zero-based)
             , Mk   # barber timings
             ):
    ret = 0
    alloc = [-1 for i in range(B)]
    t = 0
    b = -1
    ...
    if gcd > 1:
        ...
        repeat = sum([lcm // Mk[i] for i in range(B)])
        N %= repeat
...
#     inputFileName = '-large-practice.in'
    ...
    while testCaseNumber <= testCaseCount:
        ...
        print('Case #%d: B=%d, N=%d, Mk=[%s],' % (testCaseNumber, B, N, showString(textLine, 60))),
