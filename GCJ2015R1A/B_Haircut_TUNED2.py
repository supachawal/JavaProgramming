...
You are waiting in a long line to get a haircut at a trendy barber shop. The shop has B barbers on duty, 
and they are numbered 1 through B. It always takes the kth barber exactly Mk minutes to cut a customer's hair, 
and a barber can only cut one customer's hair at a time. Once a barber finishes cutting hair, he is immediately free 
...
Input: The first line of the input gives the number of test cases, T. T test cases follow; each consists of two lines. 
...
def getBarber(B     # barber count
    ...
    #2^N walk to exponentially converge
    t = 0
    step = 1
    new_t = 0
    servedCustomerCount = 0
    while servedCustomerCount < customerIndex - B or servedCustomerCount > customerIndex:
        #ELAPSED=151.595594
        if new_t >= 0:
            t = new_t
            new_t = -1
        t += step
        servedCustomerCount = sum([t // mki + 1 for mki in Mk])
        if servedCustomerCount > customerIndex:
            new_t = t - step
            step = 1
        else:
            step *= 2

    b = servedCustomerCount - 1
...
#     inputFileName = '-small-practice.in'
    ...
    while testCaseNumber <= testCaseCount:
        ...
        print('Case #%d: B=%d, N=%d, Mk=[%s],' % (testCaseNumber, B, N, showString(textLine, 70)), end=' ')
