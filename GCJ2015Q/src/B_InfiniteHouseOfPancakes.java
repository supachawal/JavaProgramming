...
//			inputFileName = "-large-practice.in";
			inputFileName = B_InfiniteHouseOfPancakes.class.getSimpleName().substring(0, 1) + inputFileName;
		}
		...
		try {
			...
			B_InfiniteHouseOfPancakes solver = new B_InfiniteHouseOfPancakes();
...
//				4					
	...
	private int _diningSpecialMinute(int maxPi) {
		...
		for (int i = (maxPi + 1) >> 1; i + freq < maxPi && improving && loopCount < 2; i++) {
			iterationCounter++;
			loopCount++;
			final int TARGET = 0; 
...
//				if ((maxPi & 1) == 0 && improvementCount > 1) {
//					System.out.printf("Oh! NOOOOOOOOOOOOOOOO\n");
//				}
				if ((maxPi & 1) == 0) {
					improving = false;		//Case #11: {1000}, maxPi=1000, answer=63, visits=222887140
				}
			} else {
				improving = false;
			}
