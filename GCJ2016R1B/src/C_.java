...
//			inputFileName = "-large-practice.in";
			...
			inputFileName = C_.class.getSimpleName().substring(0, 1) + inputFileName;
		}
	...
	public void solve(final File aFile, PrintWriter w) throws FileNotFoundException, IOException {
		...
		while (++testCaseNumber <= testCaseCount && (textLine = br.readLine()) != null) {
			...
			if (debuff == 0) {
				...
				int defences1 = Hd <= 2 * Ak ? Integer.MAX_VALUE : minDefence(Ak, nMinPeriodsToKill - 2, Hd - Ak);
				memoCache = new HashMap<IntPack, Integer>();
				int defences2 = minDefence(Ak, nMinPeriodsToKill, Hd);
	...
	private int minDefence(int Ak, int n, int currentHd) { //assumes debuff > 0
		...
		if (currentHd > debuffedAk) { //debuff
			ret = Math.min(ret, 1 + minDefence(debuffedAk, n, currentHd - debuffedAk));
		}
		
		if (currentHd <= Ak) { // cure
			if (fullHd > 2 * Ak) {
				ret = Math.min(ret, 1 + minDefence(Ak, n, fullHd - Ak));
			}
		} else {	// do nothing, let the enemy attack ******** TOO SLOW
			ret = Math.min(ret, minDefence(Ak, n - 1, currentHd - Ak));
		}
